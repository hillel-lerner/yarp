...
from yarp.yarpecule.input_parsers import (
    ...
    load_reactions_from_xyz_directory,
...
def generate_rxns(inp):
    ...
    if inp.enum.ON:
        ...
        if inp.init_struct.mode == "species":
            if verbose:
                ...
                print(
                    f" - Processing starting node as a single species. No pre-enumeration filters will be applied!"
                )
            ...
            raw_products = enumerate_products(
                r_yp=reactant,
                n_break=inp.enum.n_break,
                n_form=inp.enum.n_form,
                react=inp.enum.react_atoms,
                mode=inp.enum.mode,
                verbose=verbose,
            )
            ...
            clean_products = filter_enum_products(
                raw_products,
                l_cutoff=inp.enum.post_enum_filters.lewis_score,
                fc_cutoff=inp.enum.post_enum_filters.formal_charge,
                ring_filter=inp.enum.post_enum_filters.ring_filter,
                verbose=verbose,
            )
            ...
            for prod in clean_products:
                ...
                if opt_prod is None:
                    ...
                    if verbose:
                        print(
                            f"    + SKIPPED! Unable to form valid product ({prod.canon_smi}) geom from reactant ({reactant.canon_smi}) geom"
                        )
                    continue
        ...
        if inp.init_struct.mode == "reaction":
            if verbose:
                print(
                    f" - Initializing starting reactant node(s) from {inp.init_struct.source}"
                )
                print(
                    f" - Initial source identified as reactant(s); pre-enumeration filters will be applied!"
                )
            ...
            if inp.init_struct.type == "yarp_pickle":
                if verbose:
                    print(
                        " - Processing starting node(s) as YARP generated pickle file"
                    )

                og_rxns = pickle.load(open(inp.init_struct.source, "rb"))
                assert isinstance(og_rxns, dict), (
                    "Input pickle file must contain a dictionary!"
                )
                assert all(isinstance(v, reaction) for v in og_rxns.values()), (
                    "YARP requires a dictionary of reaction objects to continue"
                )
                ...
                if source.is_dir():
                    if verbose:
                        print(
                            f" - Processing starting node(s) as reaction xyz files in {source}"
                        )
                    og_rxns = load_reactions_from_xyz_directory(source)
                elif source.is_file() and source.suffix.lower() == ".xyz":
                    if verbose:
                        print(
                            f" - Processing starting node(s) as reaction xyz file {source}"
                        )
                ...
                if verbose:
                    print(
                        f" - Processing starting node(s) as mapped reaction SMILES in {source}"
                    )
                og_rxns = load_reactions_from_smiles_file(source)

            else:
                raise RuntimeError(
                    "We can only start from a YARP pickle file, a reaction xyz file, a directory of reaction xyz files, or a mapped reaction SMILES file currently, sorry friend!"
                )
            ...
            candidates = filter_enum_candidates(
                og_rxns,
                separate_prods=inp.enum.pre_enum_filters.separate_prods,
                prop_filter=inp.enum.pre_enum_filters.property_filter,
                netconfig=inp.enum.pre_enum_filters.product_blinders,
                verbose=verbose,
            )
            ...
            for mol in candidates:
                ...
                for prod in clean_products:
                    ...
                    if opt_prod is None:
                        ...
                        if verbose:
                            print(
                                f"    + SKIPPED! Unable to form valid product ({prod.canon_smi}) geom from reactant ({mol.canon_smi}) geom"
                            )
                        continue
    ...
    # Initialize reactions characterization without product enumeration
        ...
        print(
            f"Product enumeration not enabled. Initializing reactions from input node(s)."
        )
        ...
        if inp.init_struct.type == "yarp_pickle":
            ...
            assert all(isinstance(v, reaction) for v in output.values()), (
                "YARP requires a dictionary of reaction objects to continue"
            )
...
def quick_geom_opt(molecule, lot="uff"):
    ...
    # If RDKit could not build a force field (e.g. unparameterized atom
    # types), fall through to Open Babel rather than failing outright
    if rd_opt_g is None:
        rd_diff = None
    else:
    ...
    # If RDKit failed or generated a garbage geom, try Open Babel
    if rd_diff is None or not np.all(rd_diff == 0):
